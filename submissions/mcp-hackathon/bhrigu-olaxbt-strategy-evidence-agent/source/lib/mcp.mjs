...
import { buildStrategyEvidence } from "./strategy-evidence.mjs";
...
const SERVER_INFO = Object.freeze({
  name: "bhrigu-olaxbt-strategy-evidence-agent",
  title: "BHRIGU OlaXBT Strategy Evidence Agent",
  version: "0.3.0"
});
const SERVER_CAPABILITIES = Object.freeze({ tools: Object.freeze({ listChanged: false }) });
const SERVER_INSTRUCTIONS = "Primary trading-track job: use bhrigu_get_olaxbt_strategy_evidence to inspect evidence context behind the current OlaXBT strategy signal. OlaXBT is strategy + signal authority; BHRIGU is read-only evidence interpretation. Inherited Bitcoin temporal-evidence tools remain available. No trading, wallet, payment, or second-signal authority.";
...
export const MCP_TOOLS = Object.freeze([
  {
    name: "bhrigu_get_olaxbt_strategy_evidence",
    title: "Get OlaXBT strategy evidence context",
    description: "Read the current OlaXBT BTC/USDT strategy signal together with strategy metrics, recent trades, equity context, contradictions, limitations, and a bounded evidence-quality assessment. OlaXBT remains signal authority; BHRIGU does not create a second signal or execute trades.",
    inputSchema: {
      type: "object", additionalProperties: false, required: ["symbol"],
      properties: { symbol: { type: "string", enum: ["BTC/USDT"] } }
    },
...
export async function handleMcpRpc(body, { fetchImpl = fetch, now = new Date(), transportMeta = {} } = {}) {
  ...
  try {
    if (name === "bhrigu_get_olaxbt_strategy_evidence") {
      const symbol = args?.symbol;
      if (symbol !== "BTC/USDT") {
        return { status: 200, body: rpcResult(id, toolError("UNSUPPORTED_SYMBOL", "symbol must be BTC/USDT", modern)) };
      }
      return { status: 200, body: rpcResult(id, toolResult(await buildStrategyEvidence({ symbol, fetchImpl, now }), modern)) };
    }
